...
run_base() {
    echo "运行 ch4 基础测试..."
    echo -e "${YELLOW}────────── cargo run --features base 输出 ──────────${NC}"

    # 使用 tee 将基础测例输出同时显示在终端和传递给 tg-rcore-tutorial-checker
    if CARGO_TARGET_RISCV64GC_UNKNOWN_NONE_ELF_RUNNER="$HEADLESS_RUNNER" cargo run --features base 2>&1 | tee /dev/stderr | tg-rcore-tutorial-checker --ch 4; then
        echo ""
...
run_exercise() {
    ...
    if CARGO_TARGET_RISCV64GC_UNKNOWN_NONE_ELF_RUNNER="$HEADLESS_RUNNER" cargo run --features exercise 2>&1 | tee /dev/stderr | tg-rcore-tutorial-checker --ch 4 --exercise; then
        echo ""
        echo -e "${YELLOW}────────── 测试结果 ──────────${NC}"

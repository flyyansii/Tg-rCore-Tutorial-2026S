...
## 十六、按 Guide 讲 ch2 的 30 步流程版

这一版适合放进正式学习笔记里，因为它不只列概念，而是按 Guide 的教学顺序把机制串起来。

1. 先从 ch1 的内核启动能力出发：内核已经能在 QEMU 上运行。
2. ch2 新增目标：让内核运行用户程序，而不是只运行自己。
3. 用户程序目录 `user/` 出现，说明应用和内核开始分离。
4. `user/src/bin` 中每个文件代表一个可以被批处理系统运行的应用。
5. `user/src/lib.rs` 提供用户态运行库，负责 `_start`、清 `.bss`、调用 `main`。
6. `user/src/syscall.rs` 提供用户态 syscall 包装，负责发起 `ecall`。
7. `os/build.rs` 在编译期扫描用户程序。
8. `build.rs` 生成 `link_app.S` 或组件化版本的 `app.asm`。
9. `link_app.S/app.asm` 记录 app 数量和 app 起止地址。
10. `.incbin` 把 app 的二进制内容放进内核镜像。
11. 内核链接时获得 app 字节和 app 元数据。
12. QEMU 运行内核后，内核先做基础初始化。
13. 内核读取 app 元数据，知道第一个 app 的字节在哪里。
14. 内核计算 app 大小，也就是 `app_end - app_start`。
15. 内核把 app 字节复制到指定运行地址。
16. 内核清理或覆盖目标运行区域，避免旧数据影响新 app。
17. 内核构造用户上下文，设置用户入口地址。
18. 内核设置用户栈指针。
19. 内核设置返回用户态需要的 `sstatus`。
20. 内核通过恢复上下文和 `sret` 进入用户态。
21. 用户程序从 `_start` 开始，而不是直接从 `main` 开始。
22. `_start` 完成用户态初始化后调用 `main`。
23. 用户 `main` 如果打印，进入 `println -> sys_write -> ecall`。
24. `ecall` 让 CPU 切回 S-mode，进入内核 Trap 入口。
25. Trap 入口保存用户寄存器到 TrapContext。
26. 内核根据 `scause` 判断这是系统调用。
27. 内核根据 `a7` 判断具体 syscall 类型。
28. 如果是 `write`，内核输出并返回当前用户程序。
29. 如果是 `exit`，内核结束当前用户程序。
30. 批处理系统继续加载下一个 app，直到全部执行完。

这 30 步里最关键的不是“代码多”，而是三个边界被建立起来：

```text
编译边界：用户程序先被打包成内核镜像里的数据。
运行边界：内核复制 app，并通过 sret 进入 U-mode。
服务边界：用户程序通过 ecall 请求内核，而不是直接调用内核。
```

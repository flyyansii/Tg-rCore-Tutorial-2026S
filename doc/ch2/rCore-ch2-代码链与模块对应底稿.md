...
## 15. 流程再细化：ch2 一次用户程序从“源码”到“退出”的 30 步

这一版不是问答，而是按时间顺序走完整流程。读的时候可以把它当成一条调用链：前一格做完，后一格才能发生。

1. 我先在 `tg-rcore-tutorial-user/src/bin` 中写一个用户程序，例如 `hello_world.rs`。
2. 这个用户程序只写自己的业务逻辑，不直接写 SBI，也不直接操作 QEMU 硬件。
3. 用户程序依赖 `user_lib`，因此它能使用 `println!`、`exit`、`sys_write` 这些用户态封装。
4. 构建 ch2 内核时，`build.rs` 会先确定本章要打包哪些用户程序。
5. `build.rs` 读取配置后，为每个用户程序调用 cargo，把它编译成 RISC-V 目标文件。
6. 用户程序编译时链接自己的 `user_lib`，因此最终二进制里包含 `_start -> main -> exit` 的用户态生命周期。
7. 用户程序的链接脚本会规定它希望从哪个用户地址开始运行。
8. 编译完成后，`build.rs` 拿到用户程序的 ELF 或二进制字节。
9. `build.rs` 生成一份汇编文件，Guide 里叫 `link_app.S`，组件化里更像自动生成的 `app.asm`。
10. 这份汇编文件用 `.incbin` 把用户程序字节嵌进内核镜像。
11. 汇编文件同时生成 app 起止符号，让内核运行时知道每个 app 的边界。
12. `main.rs` 通过 `global_asm!(APP_ASM)` 把这份汇编文本纳入内核编译。
13. 链接器把内核代码段、数据段和 app 字节一起排进最终内核镜像。
14. QEMU 启动后，CPU 进入内核入口，ch1 中的裸机启动链仍然负责把内核跑起来。
15. 内核进入 `rust_main` 后先清理 `.bss`，初始化 console、log 和 syscall 环境。
16. 内核调用 `AppMeta::locate()` 找到刚才嵌入镜像的 app 元数据。
17. 元数据告诉内核：本章有几个 app、每个 app 在镜像中的起止位置、运行基址和间隔。
18. 批处理循环取出第一个 app 的字节范围。
19. 内核把 app 从“内核镜像中的存放位置”复制到“用户程序的运行位置”。
20. 复制完成后，内核构造用户上下文，把用户 PC 设置到 app 入口。
21. 内核给用户上下文设置用户栈，让 app 可以正常调用函数。
22. 内核设置 `sstatus`，保证 `sret` 后 CPU 从 S-mode 回到 U-mode。
23. 内核执行上下文恢复路径，最终用 `sret` 把控制权交给用户程序。
24. 用户程序从 `_start` 开始，先清自己的 `.bss`。
25. `_start` 调用用户写的 `main()`，用户程序正式执行。
26. 用户执行 `println!` 时，先进入用户态格式化输出逻辑，再调用 `sys_write`。
27. `sys_write` 把 syscall id 和参数放进 `a7/a0/a1/a2`，然后执行 `ecall`。
28. CPU 根据 `stvec` 跳回内核 Trap 入口，内核保存用户现场并分发 syscall。
29. 如果 syscall 是 `write`，内核输出后修改返回值和 `sepc`，再回到用户程序继续。
30. 如果 syscall 是 `exit`，内核不再恢复当前 app，而是回到批处理循环加载下一个 app。
...
## 16. ch2 `write` 系统调用的 28 步流程

这一条链专门用于讲“为什么用户打印一句话要绕这么远”。

1. 用户程序执行 `println!("hello")`。
2. `println!` 宏先把格式化内容交给用户库中的输出对象。
3. 用户库的输出对象实现 `core::fmt::Write`。
4. `write_str` 收到字符串切片。
5. `write_str` 调用用户态 `write(fd, buf, len)`。
6. `fd=1` 表示标准输出。
7. `buf` 是用户地址空间中的字符串地址。
8. `len` 是要写出的字节数。
9. `write` 调用 `sys_write`。
10. `sys_write` 调用通用 `syscall`。
11. `syscall` 把 `SYS_WRITE` 放入 `a7`。
12. `syscall` 把 `fd` 放入 `a0`。
13. `syscall` 把 `buf` 放入 `a1`。
14. `syscall` 把 `len` 放入 `a2`。
15. 内联汇编执行 `ecall`。
16. CPU 从 U-mode 切到 S-mode。
17. CPU 把 Trap 原因写入 `scause`。
18. CPU 把用户 PC 写入 `sepc`。
19. CPU 跳到 `stvec` 指向的内核 Trap 入口。
20. Trap 入口保存用户通用寄存器，形成 TrapContext。
21. 内核进入 syscall 分发逻辑。
22. 内核读取 `a7`，确认这是 `write`。
23. 内核读取 `a0/a1/a2`，拿到 fd、用户 buffer 和长度。
24. ch2 没有页表，所以内核可以直接按地址读用户 buffer。
25. 内核把 buffer 中的字节交给 console 输出。
26. console 通过 SBI 或 QEMU 串口显示字符。
27. 内核把返回值写回 `a0`，并让 `sepc += 4`。
28. 内核执行返回路径，`sret` 回到用户程序，`println!` 完成。

...
## 19. 讲述用流程：从“轮流运行”展开成 30 步

如果我要把 ch3 讲给完全没写过 OS 的同学，我会避免一上来就说一堆上下文，而是从“轮流运行”展开：

1. 想让多个程序轮流运行，首先要同时记住多个程序。
2. 记住一个程序，就需要一个 TCB。
3. 记住多个程序，就需要任务表。
4. 管理任务表，就需要 TaskManager 或等价调度逻辑。
5. 一个任务正在运行时，CPU 寄存器里是它的状态。
6. 它被打断时，寄存器不能丢。
7. 所以 Trap 发生时要保存 TrapContext。
8. TrapContext 保存用户态现场。
9. 内核处理完 Trap 后，可能不立刻回这个任务。
10. 如果不回这个任务，就要切到另一个任务。
11. 切任务发生在内核态。
12. 内核态也有自己的返回地址和栈。
13. 所以切任务时还要保存 TaskContext。
14. TaskContext 保存内核切换现场。
15. 第一次运行任务时，真实现场不存在。
16. 内核提前伪造初始现场。
17. 伪造的 TrapContext 告诉 `sret` 去用户入口。
18. 伪造的 TaskContext 告诉 `ret` 去 `__restore`。
19. 用户程序主动 `yield` 时，走 syscall。
20. syscall 先保存 TrapContext。
21. 调度器再决定切走。
22. `__switch` 保存 TaskContext。
23. `__switch` 恢复另一个 TaskContext。
24. 另一个任务通过 `__restore` 回到用户态。
25. timer 中断是不主动 yield 时的强制版本。
26. exit 是任务生命周期结束。
27. fault 是任务出错被内核杀死。
28. write/read 是普通服务请求，不一定触发切换。
29. trace 是观察当前任务 syscall 历史的调试接口。
30. ch3 的重点就是让这些情况都能被调度器统一管理。
